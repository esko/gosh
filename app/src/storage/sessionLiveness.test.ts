...
describe('sessionLiveness', () => {
  it('marks a host live within the freshness window and dead after it', () => {
    const t0 = 1_000_000;
    recordHeartbeat('et:user@host:22:2022', 'et', t0);
    expect(isHostLive('et:user@host:22:2022', t0 + 1_000)).toBe(true);
    expect(isHostLive('et:user@host:22:2022', t0 + FRESHNESS_MS - 1)).toBe(true);
    expect(isHostLive('et:user@host:22:2022', t0 + FRESHNESS_MS + 1)).toBe(false);
  });
