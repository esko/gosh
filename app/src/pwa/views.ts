  ...
  openOverlay((close) => {
    ...
    const modal = elFromHTML(`
      <div class="modal">
        ...
        <form id="connForm">
          <label class="field"><span>address</span><input name="host" placeholder="192.0.2.10" autocomplete="off" spellcheck="false" required></label>
          <div class="field-row">
            <label class="field"><span>user</span><input name="user" placeholder="user" autocomplete="off" spellcheck="false" required></label>
...
async function renderAppearanceTab(body: HTMLElement, profileId: string): Promise<void> {
  ...
  const swatches = [...THEME_PRESETS.entries()]
    .map(([id, p]) => {
      ...
      const chip = `<span class="theme-chip" style="background:${p.background};color:${p.foreground}"><span style="color:${p.green}">user</span>@<span style="color:${p.blue}">host</span> $ ls
  ...
  root.innerHTML = `
    <div class="connect-page">
      <form id="terminalConnect">
        ...
        <label class="field"><span>address</span><input id="terminalCommand" name="host" placeholder="user@host.example or mosh user@host" autocomplete="off" spellcheck="false" autofocus></label>
