...
- `/terminal` as the native new-tab target.
- `iwa-ssh` profiles and recents as the connection launch model.
- One terminal connection per native app tab.
...
2. As a ChromeOS user, I want the native new-tab button to open `/terminal`, so that ChromeOS/IWA tabs are the only tab model.
3. As a terminal user, I want `/terminal` to show a connect form when no connection was provided, so that a new app tab can start a connection without returning home.
...
- Browser smoke covers `/` and `/terminal`.
...
An initial replacement slice is implemented in the working tree under `app/src/pwa/`. It boots `/` and `/terminal`, uses Ghostty-web, supports profile-based launch, has echo smoke transport, and wires SSH through the existing Direct Sockets/nassh runtime.
