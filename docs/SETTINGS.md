...
- `Ctrl+Shift+H`: toggle the custom title bar (caption, tabs, window controls).
- `Ctrl+Shift+C` / `Ctrl+Shift+V` / `Ctrl+Alt+Shift+V`: copy / paste / paste
  ...
  Uploads land in the settings **Image paste directory** (default `/tmp`;
  absolute or home-relative). SSH, Mosh, ET, and tsshd all use the profile's
  SSH endpoint for the SFTP/exec sidecar.
...
ligatures, Nerd Font, Unicode symbols), cursor, and window (padding, scroll speed, scrollback,
accent, density, hide title bar). Keyboard covers the shortcut registry
(rebind/reset), the ChromeOS pass-through list, and copy/paste behavior
(copy-on-select, Ctrl+Shift copy/paste, right/middle-click paste). Behavior
covers session (confirm-close, close-on-exit) and terminal (TERM type, bell).
Settings profiles persist in `localStorage` and are resolved per pane session;
the model is `PwaTerminalSettings` (`app/src/pwa/types.ts`). Keybindings, the
pass-through list, title-bar visibility, and the Diagnostics debug pill are
app-global (not per settings-profile), persisted under the `gosh-keybindings`,
`gosh-passthrough-shortcuts`, `gosh-titlebar-hidden`, and `gosh-debug-hud`
localStorage keys. The debug pill is off by default; `?debug=1` forces it on
for a single load.
